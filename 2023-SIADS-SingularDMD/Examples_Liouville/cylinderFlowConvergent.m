...
%% Liouville DMD
[~,~,~,ReconFun,VectorField] = ...
    ConvergentLiouvilleEigenDMD(K,KT,Trajectories,SampleTime,GramRegParam);

%% Video
if ShowVideo || SaveVideo
    if SaveVideo
        filename = ['cyl-' K.type 'liouville-conv-reg-' num2str(GramRegParam) '-reconstruction.mp4'];
    ...
    for i = 1:TotalLength
        t = (i-1)*h;
        ZZ = reshape(real(ReconFun(t,InitialState)),[],Width);
        subplot(2,1,1)
...
if PrintRecon
    ...
    y = 1:1:Dimension/Width;
    [XX,YY] = meshgrid(x,y);
    ReconIndices = [10 55 80 113 150];
    load([DATAPATH '/CCcool.mat']) 
    InitialState = Trajectories(1:end,1,1);
    for ii=1:numel(ReconIndices)
        i = ReconIndices(ii);
        t = (i-1)*h;
        ZZ = reshape(real(ReconFun(t,InitialState)),[],Width);
        fig1 = surf(XX,YY,real(ZZ));
        ...
        if(PrintRecon == 1)
            ...
            set(gca,'YTickLabel',[]);
            set(gca,'XTickLabel',[]);
            filename = ['cyl-' K.type 'liouville-conv-reg-' num2str(GramRegParam) '-reconstruction-at-' num2str(t) '.pdf'];
            saveas(gcf,filename);
        end
        ZZ = reshape(Snapshots(1,:,i),[],Width);
        fig1 = surf(XX,YY,real(ZZ));
...
if PlotModes
    ...
    for i = 1:TotalModesToDisplay
        ...
        if(PrintModes == 1)
            ...
            set(gca,'YTickLabel',[]);
            set(gca,'XTickLabel',[]);
            filename = ['cyl-' K.type 'liouville-conv-reg-' num2str(GramRegParam) '-real-' num2str(i) '-mode-' num2str(s) '.pdf'];
            saveas(gcf,filename);
        end
        ...
        if(PrintModes == 1)
            ...
            set(gca,'YTickLabel',[]);
            set(gca,'XTickLabel',[]);
            filename = ['cyl-' K.type 'liouville-conv-reg-' num2str(GramRegParam) '-imag-' num2str(i) '-mode-' num2str(s) '.pdf'];
            saveas(gcf,filename);
        end
    end   
