...
function f_nummarkers3(h,num, doSpacingAlongX)
if nargin < 3
    doSpacingAlongX = 1;
end
...
for n = 1:length(h)
    if strcmp(get(h(n),'type'),'line')
        ...
        if (doSpacingAlongX)
            ...
            t = 1:length(x);
            s = [0 cumsum(sqrt(diff(x).^2+diff(y).^2+diff(z).^2))]; %measures length along the curve
        ...
        set(line(xi,yi,zi),'marker',marker,'linestyle','none','color',color,'markerfacecolor',color,'markersize',ms);
